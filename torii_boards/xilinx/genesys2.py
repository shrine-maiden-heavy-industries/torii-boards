...
class Genesys2Platform(XilinxPlatform):
	...
	def __init__(self, JP6 : Literal['1V2', '1V8', '2V5', '3V3'] = '2V5') -> None:
		...
		if JP6 not in ('1V2', '1V8', '2V5', '3V3'):
			raise ValueError(f'JP6 must be one of \'1V2\', \'1V8\', \'2V5\', \'3V3\', not {JP6!r}')

		self._JP6 = JP6

	def bank15_16_17_iostandard(self) -> str:
	...
	def toolchain_program(self, products : BuildProducts, name : str) -> None:
		...
		with products.extract(f'{name}-openocd.cfg', f'{name}.bit') as files:
			...
			check_call([
				openocd,
				'-f', config_filename,
				'-c', f'init; pld load 0 {bitstream_filename}; exit'
			])
